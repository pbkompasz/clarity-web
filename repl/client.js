...
on_ready(() => {
  repl_ready = true;
  queued_evaluations.forEach(([code, result_container, options]) =>
    eval_clarity_code(code, result_container, options)
  );
  queued_evaluations = [];
});
...
async function eval_clarity_code(code, result_container, options) {
  ...
  if (runtime_error) {
    const match = result.match(
      /^Runtime Error: ShortReturn\(AssertionFailed\(Sequence\(String\("(.+?)"\)\)\)\)$/
    );
    if (match && match[1]) result = match[1];
  }
  const events =
    !analysis_error &&
    result.indexOf("Events emitted") !== -1 &&
    result
      .match(/^({.+?})$/gm)
      .map((entry) => json_parse_safe(entry))
      .filter((r) => !!r);
  const deploy_success =
    !events &&
    result.indexOf(
      "contract successfully stored. Use (contract-call? ...) for invoking the public functions:"
    ) !== -1;
    ...
    result_container.classList.add(
      !analysis_error &&
        ...
        (!options.expected_output ||
          result === options.expected_output ||
          (events.length &&
            events[0].contract_event &&
            events[0].contract_event.value === options.expected_output))
        ? "pass"
        : "fail"
    );
  }
